...
Page {
    ...
    // Sailjail hides from the target app (it would see 0 bytes / "corrupt"). Stage a
    // world-readable copy under ~/Downloads/sfmail/ first, then open THAT path.
    function _openWith(src, name) {
        var p = Gpg.stageForOpen(src, name)
        if (p && p.length > 0)
    ...
    // anything that would load external content or run code. Qt's RichText renderer
    // has no JavaScript, so scripts are inert anyway; the real risk is <img src=http…>
    // (tracking pixels) which the renderer WOULD fetch — those are removed. Inline
    // data: images are already part of the message, so they are kept.
    function _simpleHtml(html) {
        var s = "" + html
        ...
        s = s.replace(/\son\w+\s*=\s*("[^"]*"|'[^']*'|[^\s>]*)/gi, "")  // on* handlers
        // Readability on a phone: drop the mail's own tiny font sizes and fixed
        // pixel widths/heights so the text uses our base font and reflows to the
        // screen width instead of rendering as a shrunken desktop layout.
        s = s.replace(/font-size\s*:\s*[^;"']*/gi, "")          // CSS font-size
        s = s.replace(/\bsize\s*=\s*("[^"]*"|'[^']*'|[^\s>]*)/gi, "")  // <font size=>
        s = s.replace(/\b(width|height)\s*=\s*("[^"]*"|'[^']*'|[^\s>]*)/gi, "")  // attrs
        s = s.replace(/\b(?:max-|min-)?(?:width|height)\s*:\s*[^;"']*/gi, "")    // CSS
        // Linearize table layouts. E-mail uses <table> purely for layout; Qt's
        // RichText renderer squeezes a cell below one word's width on a narrow
        // (portrait) screen, which then breaks text one character per line. Flatten
        // tables to normal flow: rows become line breaks, cells become spaces.
        s = s.replace(/<\/?(?:table|tbody|thead|tfoot|colgroup|col)\b[^>]*>/gi, "")
        s = s.replace(/<tr\b[^>]*>/gi, "<br>")
    ...
    function _htmlToText(html) {
        ...
        s = s.replace(/<(br|\/p|\/div|\/tr|\/h[1-6])\b[^>]*>/gi, "\n")
        s = s.replace(/<[^>]+>/g, " ")
    ...
    // (and from other clients) carries "..." in the visible header and the true
        ...
        Column {
            ...
            BackgroundItem {
                ...
                Label {
                    ...
                    text: page._showHtml
                          ? qsTr("Show plain text")
                          : qsTr("Show simple HTML (no external content loaded)")
                    color: Theme.highlightColor
                    font.pixelSize: Theme.fontSizeSmall
                }
            }
                    ...
                    delegate: Column {
                        ...
                        ListItem {
                            ...
                            menu: ContextMenu {
                                MenuItem {
                                    text: qsTr("Open with…")
                                    onClicked: page._openWith(decAtt.attSrc, decAtt.attName)
                                }
                                MenuItem {
                                    text: qsTr("Save as…")
